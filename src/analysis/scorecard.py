...
def build_scorecard(reg, regime, risk_df: pd.DataFrame, *, risk_weight: float = 2.0 , spy_ann: float = 13.0,  val_table: pd.DataFrame | None = None, prof_summary: dict | None = None, syn_summary: dict | None = None) -> list[dict]:
    ...
    if val_table is not None:
        vp = val_table[val_table['group'] == 'pure_play']
        med_cagr = vp['implied_required_cagr_%'].median()
        if pd.notna(med_cagr):
            v = 1 if med_cagr < 20 else (-1 if med_cagr > 50 else 0)
    ...
    if syn_summary is not None:
        earned = syn_summary.get("earned", 0)
        narrative = syn_summary.get("narrative", 0)
        total = syn_summary.get("pure_play_total", 0)
        if earned + narrative > 0: 
            v = 1 if earned > narrative else (-1 if narrative > earned else 0)
            add("Earned vs Narrative", v, f"{earned} earned, {narrative} narrative out of {total}", 2.0, "return")
        else:
    ...
    if prof_summary is not None:
        ...
        if up+ down > 0:
            ...
            add("Margins Improving", v, f"{up} improving vs {down} deteriorating", 1, 'return')
        else:
...
def compute_verdict(signals: list[dict]) -> dict:
    ...
    positive_thresh = max_possible * 0.3
    negative_thresh = -positive_thresh
    ...
    if score >= positive_thresh:
        ...
        verdict = "Real, not yet derisked"
    else:
        verdict = "The evidence is mixed"

    return{ "score": score, "risk_score": risk_score, "return_score": return_score, "n_decisive": n_decisive, "max_possible": max_possible, "verdict": verdict }